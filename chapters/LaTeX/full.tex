...
\subsubsection{Aritmética}\label{aritmuxe9tica}}

Com \texttt{int} e \texttt{float} é possível utilizar os operadores
comuns da Matemática. Quando ocorre entre eles, o resultado será um
\texttt{float}.
...
\tightlist
\item
  \texttt{+}/\texttt{-} para adição/sutração.
\item
  \texttt{*}/\texttt{/} para multiplicação/divisão.
\item
  \texttt{**} para potenciação.
\item
  \texttt{\%} para módulo (resto).
\end{itemize}
